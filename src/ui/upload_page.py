...
try:
    from .data_preview import DataPreviewWidget, FileInfo, FileType, PreviewConfig
except ImportError:
    # 如果导入失败，提供最小的定义
    from enum import Enum
    @dataclass
    ...
    class DataPreviewWidget(QWidget):
        ...
        def load_file_data(self, file_info, data):
            pass
...
class UploadPage(QWidget, LoggerMixin):
    ...
    def _setup_ui(self):
        ...
        if HAS_FLUENT_WIDGETS:
            upload_card.viewLayout.addLayout(upload_layout)
        ...
        # 将内容布局添加到HeaderCardWidget（仅在使用Fluent Widgets时）
        if HAS_FLUENT_WIDGETS:
            self.info_card.viewLayout.addLayout(self.info_layout)
    ...
    def get_current_file_info(self) -> FileInfo | None:
        """获取当前文件信息"""
...
def create_upload_page(config: UploadConfig | None = None) -> UploadPage:
    """创建上传页面的工厂函数"""
    try:
