...
try:
    from qfluentwidgets import (
        CardWidget, HeaderCardWidget, SimpleCardWidget, ElevatedCardWidget,
        ...
        CheckBox, ComboBox, SpinBox, DoubleSpinBox,
        ProgressBar, IndeterminateProgressBar, ProgressRing,
        TextEdit, PlainTextEdit, ScrollArea, Pivot, PivotItem,
...
from ..core.chart_renderer import ChartRenderer, ChartStyle, ChartType, ChartFormat
from ..core.history_manager import get_history_manager, HistoryManager
...
from ..utils.exceptions import AnalysisError, ChartGenerationError
...
class AnalysisResultsWidget(QWidget, LoggerMixin):
    ...
    def _create_high_correlations_card(self, correlation_matrix):
        ...
        if not high_corrs:
            ...
            # 显示前10个高关联对
            for i, corr_info in enumerate(high_corrs[:10]):
                if HAS_FLUENT_WIDGETS:
...
class AnalysisPage(QWidget, LoggerMixin):
    ...
    def __init__(self, config: Optional[AnalysisPageConfig] = None, parent=None):
        super().__init__(parent)
        ...
        self.current_result: Optional[AnalysisResult] = None
        self.current_history_record: Optional[AnalysisHistoryRecord] = None
    ...
    def _create_control_widget(self):
        ...
        # 开始分析按钮
        if HAS_FLUENT_WIDGETS:
            self.start_btn = PrimaryPushButton("开始分析", FluentIcon.PLAY)
        else:
        ...
        # 导出按钮
        if HAS_FLUENT_WIDGETS:
            self.export_btn = PushButton("导出报告", FluentIcon.DOWNLOAD)
        else:
    ...
    def load_data(self, data: Any, file_path: Optional[str] = None, time_column: Optional[str] = None):
        """加载数据"""
        try:
    ...
    def _create_file_info_for_export(self):
        """为导出创建文件信息对象"""
        from ..models.file_info import FileInfo, FileType, DataQualityInfo
...
def create_analysis_page(config: Optional[AnalysisPageConfig] = None) -> AnalysisPage:
    """创建分析页面的工厂函数"""
    try:
