...
try:
    from PyQt6.QtWidgets import (
        ...
        QDialogButtonBox, QAbstractItemView, QMenu, QCheckBox
    )
    from PyQt6.QtCore import (
        Qt, QThread, pyqtSignal, QTimer, QObject, QDate,
...
try:
    from qfluentwidgets import (
        ...
        LineEdit, ComboBox, DateEdit, SpinBox,
        ProgressBar, IndeterminateProgressBar, ProgressRing,
        TextEdit, PlainTextEdit, ScrollArea, Pivot, PivotItem,
        InfoBar, InfoBarPosition, InfoBarIcon, TableWidget,
        FluentIcon, Theme, isDarkTheme, SearchLineEdit,
...
class HistoryTableModel(QAbstractTableModel, LoggerMixin):
    ...
    def get_record(self, row: int) -> Optional[AnalysisHistoryRecord]:
        """获取指定行的记录"""
...
class HistoryPage(QWidget, LoggerMixin):
    ...
    def __init__(self, config: Optional[HistoryPageConfig] = None, parent=None):
        super().__init__(parent)
        self.config = config or HistoryPageConfig()
        
        # 核心组件
        self.db: Optional[AnalysisHistoryDB] = None
        self.table_model: Optional[HistoryTableModel] = None
    ...
    def _setup_ui(self):
        ...
        # 刷新按钮
        if HAS_FLUENT_WIDGETS:
            self.refresh_btn = ToolButton(FluentIcon.REFRESH)
        else:
    ...
    def _create_control_panel(self):
        ...
        if self.config.enable_export:
            if HAS_FLUENT_WIDGETS:
                self.export_btn = PushButton("导出", FluentIcon.DOWNLOAD)
            else:
...
def create_history_page(config: Optional[HistoryPageConfig] = None) -> HistoryPage:
    """创建历史页面的工厂函数"""
    try:
