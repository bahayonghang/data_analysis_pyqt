...
try:
    from PyQt6.QtWidgets import (
        ...
        QScrollArea, QFrame, QGroupBox, QHeaderView,
        QAbstractItemView, QSizePolicy, QTextEdit
    )
...
try:
    from qfluentwidgets import (
        ...
        FluentIcon, Theme, isDarkTheme, ScrollArea
    )
...
class DataPreviewTable(QTableWidget if HAS_PYQT6 else QWidget, LoggerMixin):
    ...
    def __init__(self, config: PreviewConfig, parent=None):
        ...
        self.config = config
        self.data: Optional[Any] = None  # pandas DataFrame 或 polars DataFrame
    ...
    def _get_preview_data(self) -> Optional[Any]:
        """获取预览数据"""
...
class DataPreviewWidget(QWidget, LoggerMixin):
    ...
    def __init__(self, config: Optional[PreviewConfig] = None, parent=None):
        super().__init__(parent)
        self.config = config or PreviewConfig()
        self.file_info: Optional[FileInfo] = None
        self.data: Optional[Any] = None
        self.column_info: List[ColumnInfo] = []
        self.basic_stats: Optional[BasicStats] = None
    ...
    def _detect_time_format(self, col_data: Any) -> Optional[str]:
        """检测时间格式（pandas）"""
        try:
            if col_data.dtype == 'object':
    ...
    def _detect_time_format_polars(self, col_data: Any) -> Optional[str]:
        """检测时间格式（polars）"""
        # polars 的时间格式检测相对简单
        try:
    ...
    def get_basic_stats(self) -> Optional[BasicStats]:
        """获取基本统计信息"""
...
def create_data_preview_widget(config: Optional[PreviewConfig] = None) -> DataPreviewWidget:
    """创建数据预览组件的工厂函数"""
    try:
