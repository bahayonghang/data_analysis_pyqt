...
try:
    ...
    from PyQt6.QtCore import (
        ...
        pyqtSignal, QObject, QThread, QEvent
    )
...
try:
    from qfluentwidgets import (
        NavigationInterface, NavigationItemPosition, NavigationBarPushButton,
        qrouter, FluentIcon, Theme, setTheme, setThemeColor, isDarkTheme,
        FluentTitleBar, SplashScreen, InfoBadge, InfoBadgePosition,
        ...
        ConfigValidator, qconfig, ConfigItem, BoolValidator, ColorValidator
    )
...
from ..utils.exceptions import UIRenderError, ComponentInitializationError
...
class NavigationManager(QObject, LoggerMixin):
    ...
    def get_page_widget(self, page_id: str) -> Optional[QWidget]:
        """获取页面widget"""
...
class PageContainer(QStackedWidget, LoggerMixin):
    ...
    def _apply_responsive_layout(self):
        """应用响应式布局"""
        try:
            for page_id, widget in self.page_widgets.items():
    ...
    def remove_page(self, page_id: str):
        """移除页面"""
        if page_id in self.page_widgets:
            widget = self.page_widgets[page_id]
    ...
    def show_page(self, page_id: str, animated: bool = None, add_to_history: bool = True):
        ...
        try:
            ...
            # 添加到历史记录
            if add_to_history and old_page_id:
                self._add_to_history(old_page_id)
            ...
            # 决定是否使用动画
            use_animation = animated if animated is not None else self.animation_enabled
            
            if use_animation and old_page_id:
...
class MainWindow(MSFluentWindow, LoggerMixin):
    ...
    def __init__(self, config: Optional[UIConfig] = None):
        ...
        self.page_container: Optional[PageContainer] = None
    ...
    def _init_navigation(self):
        ...
        try:
            ...
            # 启用路由
            qrouter.setDefaultRouteKey(self.stackedWidget, NavigationPage.HOME.value)
    ...
    def add_page(
        ...
        try:
            ...
            # 注册路由
            qrouter.register(page_id, self.stackedWidget, widget)
            
            self.logger.info(f"页面已添加到主窗口: {page_id}")
            
        except Exception as e:
            self.logger.error(f"添加页面到主窗口失败: {page_id}, 错误: {str(e)}")
            raise UIRenderError(f"添加页面失败: {str(e)}") from e
    ...
    def navigate_to(self, page_id: str):
        ...
        try:
            ...
            self.logger.error(f"导航失败: {page_id}, 错误: {str(e)}")
            raise UIRenderError(f"导航失败: {str(e)}") from e
    ...
    def _on_window_state_changed(self, state: str):
        ...
        self.logger.info(f"窗口状态改变: {state}")
    ...
    def set_theme_mode(self, mode: ThemeMode):
        ...
        try:
            ...
            self.logger.info(f"主题模式已设置: {mode.value}")
            
        except Exception as e:
            self.logger.error(f"设置主题模式失败: {str(e)}")
    
    def get_current_page(self) -> Optional[str]:
        """获取当前页面"""
        if self.navigation_manager:
            return self.navigation_manager.get_current_page()
        return None
    ...
    def get_page_container(self) -> Optional[PageContainer]:
        """获取页面容器"""
        return self.page_container
    
    def set_page_animation_type(self, animation_type: str):
        """设置页面切换动画类型"""
        if self.page_container:
            self.page_container.set_animation_type(animation_type)
    
    def enable_page_animation(self, enabled: bool = True):
        """启用/禁用页面切换动画"""
        if self.page_container:
            self.page_container.enable_animation(enabled)
    
    def go_back_page(self) -> bool:
        """返回上一页"""
        if self.page_container:
...
def create_main_window(config: Optional[UIConfig] = None) -> MainWindow:
    """创建主窗口的工厂函数"""
    try:
