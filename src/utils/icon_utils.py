...
def safe_set_icon(widget: QWidget, icon: FluentIcon | QIcon | None) -> bool:
    """
    安全地为控件设置图标
    ...
    try:
        ...
        if isinstance(icon, FluentIcon):
            # 将FluentIcon转换为QIcon对象
            # qfluentwidgets的FluentIcon有icon()方法返回QIcon对象
            try:
                if hasattr(icon, 'icon') and callable(icon.icon):
                    qicon = icon.icon()
                    widget.setIcon(qicon)
                else:
                    # 如果没有icon()方法，尝试直接设置（向后兼容）
                    widget.setIcon(icon)
                ...
                logger.error(f"设置FluentIcon时出错: {e}")
                logger.error(f"FluentIcon类型: {type(icon)}")
                logger.error(f"FluentIcon值: {icon}")
                return False
...
def get_fluent_icon(icon_name: str) -> FluentIcon | None:
    """
    根据名称获取FluentIcon枚举值
